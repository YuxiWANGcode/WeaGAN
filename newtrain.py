...
parser.add_argument('--learning_rate', type=float, default=0.00225,
                    # 0.00225
...
parser.add_argument('--traffic_file', default='data/march_all.h5',
                    help='traffic file')
parser.add_argument('--otraffic_file', default='data/march_all.h5',
                    help='otraffic file')
...
parser.add_argument('--model_file', default='data/GMAN(PeMS)_march_WeaGANbest',
                    help='save the model to disk')
parser.add_argument('--best_model_file', default='data/GMAN(PeMS)_march_all_SW4L2lr4',
                    help='save the model to disk')
parser.add_argument('--log_file', default='data/log(PeMS)_march_WeaGANbest',
                    help='log file')
...
X, TE, WE, label, is_training = model.placeholder(args.P, args.Q, N)
...
pred = model.WeaGAN(
    X, TE, SE, WE, args.P, args.Q, T, args.L, args.K, args.d,
...
for epoch in range(args.max_epoch):
    ...
    for batch_idx in range(num_batch):
        print(str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + " | epoch: " + str(
            epoch) + "first loop batch idx:" + str(batch_idx))
        start_idx = batch_idx * args.batch_size
        end_idx = min(num_train, (batch_idx + 1) * args.batch_size)
        feed_dict = {
            X: trainX[start_idx: end_idx],
            TE: trainTE[start_idx: end_idx],
            WE: trainW[start_idx: end_idx],
    ...
    for batch_idx in range(num_batch):
        print(str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + " | epoch: " + str(
            epoch) + "second loop batch idx:" + str(batch_idx))
        start_idx = batch_idx * args.batch_size
        end_idx = min(num_val, (batch_idx + 1) * args.batch_size)
        feed_dict = {
            X: valX[start_idx: end_idx],
            TE: valTE[start_idx: end_idx],
            WE: valW[start_idx: end_idx],
    ...
    if val_loss <= val_loss_min:
        ...
        for q in range(args.Q):
            mae, rmse, mape = utils.metric(testPred[:, q], otestY[:, q])
            MAE.append(mae)
            RMSE.append(rmse)
            MAPE.append(mape)
